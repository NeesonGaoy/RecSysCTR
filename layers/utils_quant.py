...
class BwnQuantizer(torch.autograd.Function):
    ...
    def forward(ctx, input, clip_val, num_bits, layerwise):
        ...
        if layerwise:
            ...
            result = input.sign().mul( m.expand(s) ) # sign:  return -1, 1 binary value.
        else:
...
# https://github.com/huawei-noah/Pretrained-Language-Model/blob/master/BinaryBERT/transformer/utils_quant.py
